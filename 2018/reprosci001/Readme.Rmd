...
Sets variables, such as the ```drake.path```, reads in speciifc functions, a ggplot theme derived form ```hrbrthemes```, plot functions the drake plan ```reproplan```.
...
Checks all primary data sources from relevant distribution sources and if not already present already downloaded, retrieves the new or updated versions. 

In this case monthly price and demand csv data files form AEMO  into a local directory set by 
``` local.path```.
