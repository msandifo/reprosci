...
plots <- function(m.data) {
  ...
  p01<-ggplot(m.data  , aes(x=1000*gdp/pop ,y=perCap,  colour=region, label= round(perCap,1)))+ 
    ...
    geom_text(data= m.data %>% subset(year==2018), size=4,
  ...
  m.data.1 = m.data  %>% subset(year>1990  & region %in% c(  "Australia", "China","US", "Canada", "Poland"))
  
  
  (p01a<-ggplot(m.data.1  , aes(x=year ,y=co2/gdp,  colour=region, label= round(perCap,1)))+ 
    geom_path(arrow = arrow(angle=25,length=unit(.081, "inches"),type = "closed"))+ 
    geom_point(data= m.data.1 %>% subset(year==2009), colour="white", size=2)+
    geom_point(data= m.data.1 %>% subset(year==2009), show.legend = F, size=1)+
    geom_text(data= m.data.1 %>% subset(year==2018), size=4,
              show.legend = F, vjust=1. ,hjust=-.05  )+
    labs(y="energy sector emissions intensity - tonnes per dollar", 
         x= "year",
         subtitle= 'Energy sector data sourced from BP, population from IMF',
         caption= "Mike Sandiford, msandifo@gmail.com\n repo: https://github.com/msandifo/reprosci -> 2018/004")+
    ...
    theme(legend.position = c(.65, .95),  
          ...
          legend.key = element_rect(colour = "transparent", fill = "transparent"),
          legend.background = element_rect(fill="transparent", size=0., colour="transparent" ),
          legend.title = element_blank()) + scale_y_log10()
    )
 ...
 p02<-ggplot( m.filt.data.ref, aes(x=year ,y=perCap,  colour=region,  label= paste0(round(perCap,0), "%")))+ 
      ...
      geom_text(data= m.filt.data.ref %>% subset(year==2018), aes(x=2018.2),size=4,
                show.legend = F,
                hjust = 0 
                #label.size=0  
                #  fontface = 'bold', 
    ...
    theme(legend.position = c(.19, .25),  
         ...
         legend.background = element_rect(fill="transparent", size=0., colour="transparent" ),
         legend.title = element_blank())+
   scale_x_continuous(limits=c(2005,2018.25),  breaks=seq(2005, 2018,2))
...
p03 <-ggplot( m.filt.data.ref, aes(x=year ,y=co2,  colour=region,  label= paste0(round(co2,0), "%")))+ 
     ...
     geom_point(data=  m.filt.data.ref %>% subset(year==2009), colour="white", size=2)+
     geom_point(data=  m.filt.data.ref %>% subset(year==2009), show.legend = F, size=1)+
     geom_text(data= m.filt.data.ref %>% subset(year==2018) %>% dplyr::mutate(co2 = ifelse(region %in% c("Japan","Germany"),NA, co2 )),
               aes(x=2018.1), size=4,
               show.legend = F, 
               hjust=0
               #  
